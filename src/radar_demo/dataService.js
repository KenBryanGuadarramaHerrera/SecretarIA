...
export async function fetchDENUEFromINEGI({ token, condition, lat, lng, radius, proxyUrl = '' }) {
  ...
  // Normalize condition for the itzamna proxy:
  // 1. Remove accents (NFD decomposition, strip combining marks)
  // 2. Lowercase
  // 3. Replace spaces with + (proxy uses + not %20)
  ...
  const conditionForProxy = normalizeCondition(condition);
  const conditionEncoded = encodeURIComponent(condition || 'todos');
   ...
   */
  const directUrl = `https://www.inegi.org.mx/app/api/denue/v1/consulta/Buscar/${conditionEncoded}/${lat},${lng}/${safeRadius}/${token}`;
  // itzamna proxy expects: ?condition=cafeterias+fuentes+sodas&lat=...&lng=...&radius=...
  const url = proxyUrl
    ? `${proxyUrl}?condition=${conditionForProxy}&lat=${lat}&lng=${lng}&radius=${safeRadius}`
    : directUrl;
